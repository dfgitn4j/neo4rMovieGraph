...
if (label_matches == 0 && relationship_matches == 0) {
  ...
  if (label_matches != 0) {
    cat("Node(s):\n")
    print.data.frame(anti_join(labels, con$get_labels(), by = 'labels'))
  }
  if (relationship_matches != 0) {
    cat("RELATIONSHIP(S):\n")
    print.data.frame(anti_join(relationships, con$get_relationships(), by = 'labels'))
  }
...
setInitialGraph <- function(qry_graph_output, title = 'Graph', use_properties_for_label=FALSE) {
  ...
  node_data<-select(node_data, one_of(c('id', 'label'))) %>% 
    unnest() %>% 
    left_join(nodeVisuals, by = c('label' = 'group')) %>% 
    rename( group = label) %>%
    inner_join(node_data, by = 'id') %>% 
  ...
  if (use_properties_for_label) { 
    # label column is display text for nodes PUT IN is.na() test to return a default
    node_data$text<-replace_na(node_data$text, 'group') # use group (neo4j :Label) if no text is defined to display in node
    node_data$label<-apply(select_at(node_data, node_data$text), 1, function(x) na.omit(x)[1])  
  } else { # use :Label for node text, e.g. Movie
    node_data$label<-node_data$group
  }
  node_data$shadow<-TRUE
  node_data$shape<-'circle'
  
  # rename startNode, endNode, and type for visNetwork
  edge_data <- rename(edge_data, from = startNode, to = endNode, label = type)
  edge_data$shadow = TRUE
  edge_data$labelHighlightBold = TRUE
  edge_data$arrows.to.scaleFactor = 2
  edge_data$arrows = "to"
